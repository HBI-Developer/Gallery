$(() => {
        ...
        if ($(ev.target).closest('.header').hasClass('header')) {
            ...
            if ($(this).hasClass("active")) {
                ...
                $(this).find(".list").slideUp(400, () => {
                    ...
                    // Remove active class from settings element
    
                    $(this).removeClass("active");

                    // Remove active class from list element in settings element
    ...
    $("body").on("click", ".settings .list .setting .title", function () {
        ...
        if ($(this).hasClass("active")) {
            ...
            $(this).addClass("active").siblings('.options').slideDown().parent().siblings().children('.title').removeClass('active').siblings(".options").slideUp();
        }
    });

    $("body").on("click", ".settings .list .setting .options .option", function () {
                    ...
                    $(".loading-screen").fadeIn(400, _ => {
                        ...
                        getLangauge(value, _ => {
                            ...
                            */
                            ...
                            let page = $('.pagination .page.active').length ? +$('.pagination .page.active').text() : 1,
                                category = $(".selection-bar .option.active").length ? $(".selection-bar .option.active").index() : 0;
